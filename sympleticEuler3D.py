...
def main():
    ...
    p1 = Particle3D(np.array([0.0,0.0,0.0]), np.array([1.0,1.0,1.0]), 1.0, "Particle 1")
    ...
    outfile.write(str(time) + str(p1.position[0]) + str(p1.position[1]) + str(p1.position[2]) + str(energy))

    # Initialise data lists for plotting later
    time_list = [time]
    pos_list = [np.array(p1.position)]
    energy_list = [energy]
